<?php
    ...
    class m_user {
        ...
        public function connexion($adresse_email, $password){
            $connexion = $this->database->prepare('SELECT * FROM utilisateur WHERE adresse_email = ? AND password = ?');
        ...
        public function verification_compte_existant($adresse_email){
            $verification_compte_existant = $this->database->prepare('SELECT adresse_email FROM utilisateur where adresse_email = ?');
        ...
        public function mise_a_jour_compte_utilisateur($id, $adresse_email, $password, $is_admin){
            $mise_a_jour_compte = $this->database->prepare('UPDATE utilisateur SET  adresse_email = ?, password = ?, is_admin = ? WHERE id = ?');
        ...
        public function mise_droit_utilisateur($id, $is_admin){
            $mise_droit_utilisateur = $this->database->prepare('UPDATE utilisateur SET  is_admin = ? WHERE id = ?');
        ...
        public function obtenir_information_utilisateur($id){
            $obtenir_information_utilisateurs = $this->database->prepare('SELECT * FROM utilisateur WHERE id = ?');
        ...
        public function creation_compte_utilisateur($adresse_email, $password, $is_admin){
            $inscription = $this->database->prepare('INSERT INTO utilisateur (adresse_email, password, is_admin) 
            values (?, ?, ?) ');
        ...
        public function lister_comptes_utilisateurs(){
            $lister_comptes_utilisateurs = $this->database->prepare('SELECT * FROM utilisateur');
        ...
        public function supprimer_compte_utilisateurs($id){
            $supprimer_compte_utilisateurs = $this->database->prepare('DELETE FROM utilisateur WHERE id = ?');
        ...
        public function is_compte_existant($adresse_email){
            $is_compte_existant = $this->database->prepare('SELECT * FROM utilisateur WHERE adresse_email = ?');
